...
void LinkedList::imprimir()     //Funcion para imprimir la lista.
{
    Node *aux = this->head;
    while (aux)
    {
...
void LinkedList::operator =(const LinkedList& B)        //Funcion para el operador de asignacion.
{
    for (int i = this->size;i>=0; i--)      //Eliminado los datos de la lista.
    {
        this->removeByPosition(i);
    }
    Node *auxB = B.head;
    for (int i = 0; i < B.size; i++)            //Insertando datos de B a la lista actual.
    {
        this->insert(auxB->getValue());
        auxB = auxB->getNext();
    }
}
